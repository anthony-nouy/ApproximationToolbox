...
%% BSplines 
knots = linspace(-1,1,2)'; % knots
s = 3 ; % degree
h = BSplinesFunctionalBasis.withExtraKnots(knots,s);
...
% BSplines basis
supp = [-1,1];
knots = linspace(supp(1),supp(2),20)'; % knots
s = 4; % degree
h = BSplinesFunctionalBasis.withExtraKnots(knots,s);
...
figure(2)
clf;
fplot(@(x) [dfun(x),f.evalDerivative(1,x)],supp)
legend('Derivative true function','Interpolation')

figure(3)
clf;
fplot(@(x) [d2fun(x),f.evalDerivative(2,x)],supp)
legend('Derivative true function','Interpolation')
