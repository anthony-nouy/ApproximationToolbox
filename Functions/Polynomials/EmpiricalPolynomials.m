...
classdef EmpiricalPolynomials < OrthonormalPolynomials
    ...
    methods
        function p = EmpiricalPolynomials(x, varargin)
            ...
            % n: integer, indicates the highest degree for which a polynomial can be computed with the stored recurrence coefficients (optional, default value: 10)
            ...
            [p.recurrenceCoefficients, p.orthogonalPolynomialsNorms] = p.recurrence(rv, varargin{:});
        end
    end
    
    methods (Static, Hidden)
        function [recurr,norms] = recurrence(r,max)
            % [recurr, norms] = recurrence(r,max)
            % Computes the recurrence coefficients used to create the polynomials of the orthonormal basis, until a condition on the inner product is not met.
            ...
            n = ceil((2*max+3)/2);
            xi = r.sample;
            G = gaussIntegrationRule(NormalRandomVariable,n);
            ...
            if cond == 0 && i-2 ~= max
                error(['Maximum degree: ', num2str(i-2) ' (', num2str(max), ' asked)']);
            end
